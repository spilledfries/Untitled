...
def create_montage(
    ...
    try:
        ...
        if fmt in ("JPEG", "BMP", "TIFF") and canvas_mode == "RGB":
            ...
            # JPEG does not support transparency, so flatten over the montage background
            opaque_bg = (bg_rgba[0], bg_rgba[1], bg_rgba[2])
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--label_case",
        choices=("original", "lower", "upper"),
        default="original",
        help="Transform filename case for labels",
    )
    ...
    p.add_argument("--label_shadow_offset", default="0,0", help="Shadow offset 'dx,dy' (e.g., '2,2')")
